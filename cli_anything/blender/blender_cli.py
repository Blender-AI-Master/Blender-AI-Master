...
@click.option("--emission-color", "-ec", default=None, help="Emission color R,G,B,A (0.0-1.0)")
@click.option("--emission-strength", "-es", type=float, default=0.0, help="Emission strength (0.0-1000.0)")
@handle_error
def material_create(name, color, metallic, roughness, specular, emission_color, emission_strength):
    """Create a new material."""
    col = [float(x) for x in color.split(",")] if color else None
    ecol = [float(x) for x in emission_color.split(",")] if emission_color else None
    sess = get_session()
    ...
    mat = mat_mod.create_material(
        ...
        emission_color=ecol, emission_strength=emission_strength,
    )
...
@handle_error
def material_assign(material, object):
    """Assign a material to an object.

    MATERIAL and OBJECT can be indices (integers) or names (strings).
    Examples:
        material assign 0 1          # material index 0 to object index 1
        material assign Metal Cube    # material named 'Metal' to object named 'Cube'
    """
    sess = get_session()
    sess.snapshot(f"Assign material {material} to object {object}")
    result = mat_mod.assign_material(sess.get_project(), material, object)
