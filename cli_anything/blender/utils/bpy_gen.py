...
def generate_full_script(
    ...
    include_render_output: bool = True,
) -> str:
    ...
    # Render output
    if include_render_output:
        lines.extend(_gen_render_output(project, output_path, frame, animation))

    return "\n".join(lines)


def generate_scene_script(project: Dict[str, Any]) -> str:
    """Generate a bpy script that rebuilds the scene without rendering."""
    return generate_full_script(
        project,
        output_path="",
        frame=None,
        animation=False,
        include_render_output=False,
    )
